...
async def replay(_, message: Message):
    ...
    Pbx = await message.reply_text("Replaying...")
    que = Queue.get_queue(message.chat.id)
    if que == []:
        return await Pbx.edit("No songs in the queue to replay!")
    await player.replay(message.chat.id, Pbx)


@Pbxbot.app.on_message(filters.command("skip") & filters.group & ~Config.BANNED_USERS)
@check_mode
@AuthWrapper
async def skip(_, message: Message):
    ...
    que = Queue.get_queue(message.chat.id)
    if que == []:
        return await Pbx.edit("No songs in the queue to skip!")
    if len(que) == 1:
    ...
    is_loop = await db.get_loop(message.chat.id)
    if is_loop != 0:
        await Pbx.edit_text("Disabled Loop to skip the current song!")
        await db.set_loop(message.chat.id, 0)
    await player.skip(message.chat.id, Pbx)


@Pbxbot.app.on_message(filters.command("seek") & filters.group & ~Config.BANNED_USERS)
@check_mode
@AuthWrapper
async def seek(_, message: Message):
    ...
    Pbx = await message.reply_text("Seeking...")
    try:
        ...
        return await Pbx.edit_text("Please enter numeric characters only!")
    que = Queue.get_queue(message.chat.id)
    if que == []:
        return await Pbx.edit_text("No songs in the queue to seek!")
    ...
    try:
        ...
        return await Pbx.edit_text("Something went wrong!")
