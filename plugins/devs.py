...
@Pbxbot.app.on_message(filters.command(["eval", "run"]) & god_users)
async def eval(_, message: Message):
    Pbx = await message.reply_text("Processing ...")
    lists = message.text.split(" ", 1)
    if len(lists) != 2:
        return await Pbx.edit_text("Received empty message!")
    cmd = lists[1].strip()
...
@Pbxbot.app.on_message(filters.command(["exec", "term", "sh", "shh"]) & god_users)
async def term(_, message: Message):
    ...
    if output:
        if len(output) > 4096:
            ...
            os.remove(filename)
            return
        await Pbx.edit(f"**Output:**\n`{output}`")
    else:
        await Pbx.edit("**Output:**\n`No Output`")

@Pbxbot.app.on_message(filters.command(["getvar", "gvar", "var"]) & god_users)
...
@Pbxbot.app.on_message(filters.command("addsudo") & god_users)
...
@Pbxbot.app.on_message(filters.command(["delsudo", "rmsudo"]) & god_users)
