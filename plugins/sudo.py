...
async def gbanned_list(_, message: Message):
    if message.command[0] == "gbanlist":
        ...
        Pbx = await message.reply_text("Fetching Gbanned Users...")
        msg = "**Gbanned Users:**\n\n"
    else:
        users = await db.get_blocked_users()
        if len(users) == 0:
            return await message.reply_text("No Blocked Users Found!")
        Pbx = await message.reply_text("Fetching Blocked Users...")
        msg = "**Blocked Users:**\n\n"
    count = 0
    for user_id in users:
        ...
        try:
            ...
            continue
    if count == 0:
        return await Pbx.edit_text(f"Nobody is in {message.command[0]}!")
    else:
        return await Pbx.edit_text(msg)


@Pbxbot.app.on_message(filters.command("logs") & Config.SUDO_USERS)
@UserWrapper
...
@Pbxbot.app.on_message(filters.command("restart") & Config.SUDO_USERS)
@UserWrapper
async def restart_(_, message: Message):
    Pbx = await message.reply_text("Notifying Chats about restart....")
