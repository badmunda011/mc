...
async def favorites(_, message: Message):
    delete = False
    Pbx = await message.reply_text("Fetching favorites ...")
    favs = await db.get_all_favorites(message.from_user.id)
    if not favs:
        await Pbx.edit_text("You dont have any favorite tracks added to the bot!")
...
@Pbxbot.app.on_callback_query(filters.regex(r"add_favorite") & ~Config.BANNED_USERS)
...
@Pbxbot.app.on_callback_query(filters.regex(r"myfavs") & ~Config.BANNED_USERS)
...
@Pbxbot.app.on_callback_query(filters.regex(r"delfavs") & ~Config.BANNED_USERS)
...
@Pbxbot.app.on_callback_query(filters.regex(r"favsplay") & ~Config.BANNED_USERS)
