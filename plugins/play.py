...
async def play_music(_, message: Message, context: dict):
    ...
    if url:
        if not ytube.check(url):
            return await Pbx.edit("Invalid YouTube URL.")
        if "playlist" in url:
            await Pbx.edit("Processing the playlist ...")
            ...
            await player.playlist(Pbx, context, song_list, video)
            return
        try:
            await Pbx.edit("Searching ...")
            result = await ytube.get_data(url, False)
        except Exception as e:
            return await Pbx.edit(f"**Error:**\n`{e}`")
        context = {
            "chat_id": message.chat.id,
            "user_id": message.from_user.id,
...
async def queued_tracks(_, message: Message):
    Pbx = await message.reply_text("Getting Queue...")
    chat_id = message.chat.id
    is_active = await db.is_active_vc(chat_id)
    if not is_active:
        return await Pbx.edit_text("No active voice chat found here.")
    collection = Queue.get_queue(chat_id)
    if not collection:
        return await Pbx.edit_text("Nothing is playing here.")
    await MakePages.queue_page(Pbx, collection, 0, 0, True)
...
async def clean_queue(_, message: Message):
    ...
    Pbx = await message.reply_text("**Cleared Queue.**")
