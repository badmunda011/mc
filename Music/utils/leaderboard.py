...
class Leaderboard:
    ...
    async def broadcast(self, Pbxbot, text, buttons):
        start = time.time()
            ...
            async for chat in chats:
                try:
                    sts, msg = await self.send_message(
                        Pbxbot.app, buttons, int(chat["chat_id"]), text
                    )
                except Exception:
                    pass
                if msg is not None:
        ...
        if failed == 0:
            await Pbxbot.logit("leaderboard", to_log)
        else:
            await Pbxbot.logit("leaderboard", to_log, self.file_name)
        os.remove(self.file_name)

    async def send_message(self, Pbxbot, buttons, chat: int, text: str):
        try:
            await Pbxbot.send_message(
                chat,
                text,
            ...
            await self.send_message(Pbxbot, buttons, chat, text)
